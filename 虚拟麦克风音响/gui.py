...
def get_vars():
    # 更新变量值
    server_vars['timestamp'] = time.time()
    server_vars['random_number'] = random.randint(1, 100)
    server_vars['request_count'] += 1
    server_vars['system_status'] = 'OK' if random.random() > 0.1 else 'ERROR'
    ...
    response_data = {
        ...
        'global_vars': {k: str(v) for k, v in global_vars.items() if not k.startswith('_')}
    }
